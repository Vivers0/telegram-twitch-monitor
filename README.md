...
Если бот развернут в регионе, где Telegram API недоступен напрямую, можно указать прокси через переменную `PROXY_URL`. Прокси применяется **только к запросам Telegram** — запросы к Twitch всегда идут напрямую. Поддерживаемые форматы:
